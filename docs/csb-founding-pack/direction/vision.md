...
- **Venue.** Coinbase Advanced Trade is the launch venue because its atomic server-side bracket order (entry + stop-loss + take-profit) is exactly the shape risk invariant #1 demands. Kraken is the named fallback if Coinbase's primary fee schedule proves materially worse than ~0.40% maker / 0.60% taker at launch volume. One venue at launch; the venue adapter is a seam, and a second adapter is its own later effort.
