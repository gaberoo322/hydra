...
| `target_needs_triage > 0` (**target GH board**, scope=target — raw `needs-triage` label count; the #3059 blocker filter applies to `ready_for_agent` only) | `needs_triage_target` | `sweep_target` (issue #3709 — Target mirror of `needs_triage_orch`; no saturation cap, it drains the lane it gates on). Carries a per-item verdict-stability guard (#3729, AND-composed with the 900s cooldown) |
| `target_needs_triage_items=626,631,...` (**target GH board** — needs-triage item NUMBERS, sorted) | `state.signals.target_needs_triage_items` | `sweep_target` per-item verdict-stability guard (#3729): suppress when EVERY current item was examined within a fixed 6h window (`state.target_triage_item_stamps`); stamp all on fire, prune the rest. A SEPARATE read from the `target_needs_triage` count, so a true count + absent/empty list = degraded → fail CLOSED (not the #3872 fail-open) |
