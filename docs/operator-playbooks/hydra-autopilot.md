...
`dev_target` carries its own variant of this same required second preamble —
see the dev_target forbidden-ending block further below (issue #4196) — because
`hydra-target-build`'s own contract legitimately delegates the build to a
nested background agent, which this flat block would forbid:
...
**`dev_target` dispatches carry a variant of the forbidden-ending preamble too
(issue #4196).** `hydra-target-build`'s Step 2 legitimately delegates the
build to a nested `Agent(run_in_background=true)` — pasting the flat
`dev_orch`/`qa_orch` "Do NOT use the Agent tool at all" ban onto `dev_target`
would directly contradict that contract. A structurally identical flat ban
already broke a class that legitimately needs the Agent tool: applying it to
`qa_orch` made a dispatch skip its Standards+Spec fan-out and review both axes
inline as one reviewer (run 8e50460f) — direct evidence that the flat ban is
not safe to reuse here either. So `dev_target` gets its own additive block —
append it verbatim, alongside (never replacing) the `dev_target` worktree-guard
variant above, for every `dev_target` dispatch:

```
## NEVER END WAITING — dev_target variant, delegation permitted but blocking (issue #4196)
This is an UNATTENDED dispatch. Nothing resumes you after your final message —
reap.py records your session's end as a completion the instant it happens,
whatever you did or didn't finish. Delegating the build to a nested
`Agent(run_in_background=true)` per hydra-target-build Step 2 is PERMITTED —
that delegation is this class's contract, not an evasion. What is FORBIDDEN
is ending your turn while that child is still running.

A background child does not keep this session alive: reap.py records
completion the instant you go quiet, whatever is still running or armed,
exactly as if you had never spawned it. After spawning the delegated child,
poll it to a terminal state in the FOREGROUND before your final message — do
not end your turn to "wait for its completion notification" or "relay its
summary once it completes". Only once the child has reached a terminal state
does your final message report its outcome: a PR is open, or a hard blocker
via ## Friction Report.

This permission is scoped to the ONE delegated build spawn hydra-target-build
Step 2 describes — it is not a blanket re-permission of backgrounding
elsewhere. A backgrounded `npm test`, or an armed Monitor used for anything
other than polling that one child, remain forbidden endings for `dev_target`
exactly as they are for `dev_orch`/`qa_orch`. Commit and push your work to the
branch BEFORE running verification (`npm test`, `npm run typecheck`), not
after — verification gates whether the PR merges, not whether the work
survives the hourly worktree-orphan-prune.
```
