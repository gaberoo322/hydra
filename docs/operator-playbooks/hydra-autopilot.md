...
| `research_target_last_fired` (epoch seconds — the `research_target` field of the `hydra:autopilot:signal-last-fired` Redis hash, read directly like `scout_last_walk_iso` above; EMPTY or `0` when the field is missing / Redis is down — read as never-fired) | `research_target_last_fired` (numeric string, merged verbatim) | the 6h minimum re-fire interval on `research_target` (issue #4611). The class stamps `signal_last_fired["research_target"]` at dispatch; decide.py suppresses re-fire while `max(in-run stamp, this folded fact) < 6h ago` — the folded fact is the tier that survives bootstrap.sh's fixed-key `signal_last_fired` rewrite, so the interval holds across context_compaction restarts (the exact re-fire #4611 observed). Fail-open: absent/empty/0/malformed → never fired → dispatch allowed |
