...
| `orch_board_signals_degraded=true` (orch lane — OR-composed across EVERY gh call feeding orch board signals this turn; a NONZERO gh exit code is the only trigger, a legitimately empty payload with exit 0 never counts — issue #4130) | `orch_board_signals_degraded` (boolean — the model merges it, same shape as `orch_backfill_idle`/`tickets_available`) | suppresses IDLE-CONCLUSION, not dispatch: `decide()` refuses to terminate `cause=idle` on a degraded turn (both the `idle_turns` cap in `_check_termination` and the idle-fallback drain — `idle_turns` is one global counter fed by BOTH lanes) and the idle fallback instead emits a heartbeat `wait` carrying `reasons: heartbeat:board-degraded` + `debug.board_signals_degraded=true` + `debug.idle_fallback="deferred-board-degraded"`, so the condition is visible in the turn record; `orch_backfill_idle` is fail-closed at the SOURCE on the same flag (row below), suppressing every backfill class it drives without per-class gates. `wall_clock` still bounds the run — degradation defers idle, it never makes the run immortal. |
| `target_board_signals_degraded=true` (target lane — OR-composed across every gh call feeding Target board signals; nonzero gh exit only — issue #4130) | `target_board_signals_degraded` (boolean, merged verbatim) | same idle-conclusion suppression as the orch flag — EITHER lane's flag suppresses, because the one global `idle_turns` counter is fed by both lanes. Pre-#4130 this flag flipped only on the all-zeros else branch (total target-board failure); #4130 makes every nonzero-exit gh call in the lane flip it too, so a PARTIAL GraphQL outage can no longer masquerade as a zeroed-but-healthy read. |
...
| `orch_backfill_idle` (same signal as above) | `orch_backfill_idle` | also drives `cleanup_orch` (issue #960) — NOT staggered, so it may co-fire with the backfill set. **Fails closed (false) when the orch board read degraded this turn** (issue #4130): the conjunction is not evaluated off a zeroed fallback payload, so a GraphQL outage can no longer fabricate an idle-board signal — every `orch_backfill_idle`-consuming class (`discover_orch`, `architecture_orch`, `cleanup_orch`, `skill_prune`) is suppressed at the source, with no per-class gates in decide.py |
