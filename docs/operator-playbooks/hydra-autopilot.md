...
| `needs_qa > 0 && fresh_needs_qa == 0` (orch GH board — `stale_needs_qa`/`fresh_needs_qa` from collect-state.sh's needs-qa staleness read, default 3h via `HYDRA_STALE_NEEDS_QA_SECONDS`) | `needs_qa_orch_stale_only` | suppresses `qa_orch` (issue #3829) — an issue that has continuously carried `needs-qa` beyond the staleness window without hydra-qa clearing it (its contract, issue #638) has survived that many busy-loop turns with no verdict posted. Deliberately keyed on GitHub label STATE, not subagent stop status: the motivating failure fanned out 9 background children, lost its worktree, posted zero verdicts, and still self-reported `success`, so an ESCALATION_POLICY-style cap would never have tripped. Absent/false is fail-open (never suppresses) — a fresh needs-qa issue among the stale ones still dispatches normally. |
