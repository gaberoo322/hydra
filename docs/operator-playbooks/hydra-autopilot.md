...
| `orch_board_signals_degraded=true` (ANY of the ten orch-lane `gh` board reads in collect-state.sh exited non-zero — board-state fallback, needs-triage items, untriaged orphans, needs-qa numbers, in-flight PRs, grill list, blocker lookup, active_dev_orch, scout saturation, arch/cleanup board) | `state.signals.orch_board_signals_degraded` (boolean, merged verbatim — same shape as `orch_backfill_idle`) | **gates dispatch** (issue #4130): decide.py suppresses BOTH `terminate:idle` paths (the idle-drain conclusion in `_check_termination` and `_rule_idle_fallback` — an unreadable board is not an empty board; a held drain takes the heartbeat wait with reason `orch board read degraded — idle conclusion suspended`, visible in the turn record) and fail-closes every `orch_backfill_idle` backfill selector (`discover_orch`, `architecture_orch`, `cleanup_orch`, `skill_prune`). Emitted unconditionally (`false` when healthy) so absence is never ambiguous. A failed read also WITHHOLDS its own counts (no legitimate-looking `0`/`none` from a non-zero gh exit) — the single deliberate exception is `active_dev_orch`, which still renders `0` (test-pinned parseable line) but is fail-closed-inert. |
