...
| `design_qa_target_due=true/false` (Target board reachable AND not saturated AND the design-language ADR glob `docs/adr/*design-language*.md` matches under the Target workspace — no ADR or API-down → `false`, fail closed; the 7d class cooldown owns the cadence; #4528) | `design_qa_target_due` | `design_qa_target` (issue #2739) — the Target visual-QA pass's due gate. Row added by #4342: collect-state.sh had emitted this signal since #2739 with no wiring row, so it was never promoted — the same middle-hop gap as `retro_run_drillable`. |
