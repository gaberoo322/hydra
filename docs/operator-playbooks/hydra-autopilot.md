...
`decide.py` emits a `terminate` action when the token budget, wall-clock limit, idle-drain turns, or failure backstop trips, when the **quota-percent budget** trips (`quota`, issue #3867 — see below), when the turn count reaches the periodic session-restart cadence (`context_compaction`, issue #3787 — default every 8 Autopilot Turns via `state.limits.context_compaction_turns`, cuts the parent session's own prompt-cache re-read cost), or when the turn is wait-only with zero occupied slots (handoff baton-pass). An idle-shaped exit while the orch board read is DEGRADED terminates under cause `board_degraded` instead of `idle` (issue #4130 — a GraphQL-only outage must never be recorded as a clean "board was empty" drain; `orch_board_signals_degraded` above). Full termination conditions and the handoff baton-pass contract (issue #1903) are in `hydra-autopilot-ops-reference.md` (sibling of this SKILL.md).
...
| `orch_board_signals_degraded` (orch GH board — TRUE when any of the four load-bearing orch reads FAILED its payload validation this turn: board counts [endpoint or gh fallback], open-PR in-flight, ready-for-agent grill walk, backfill board. A GraphQL-only GitHub outage fails all four; REST-only consumers are unaffected) | `orch_board_signals_degraded` (boolean, merged verbatim) | issue #4130 — a degraded orch snapshot means every zero is UNREAD, not empty. decide.py: an idle-shaped terminate is re-caused `board_degraded` (never `idle` — the run record must never call a blind drain a clean drain), and the `orch_backfill_idle` backfill classes (`discover_orch` including its #4114 staleness floor, `architecture_orch`, `cleanup_orch`, `skill_prune`) are suppressed for the turn. Absent/false → pre-#4130 behaviour unchanged; collect-state already fails `orch_backfill_idle` closed locally, decide.py's guard is the belt-and-braces contract. |
