...
| `orch_board_signals_degraded=true` (ANY orch-lane `gh` board read failed this turn — non-zero exit or an empty payload where the query must print at least `[]`; folded over every orch read: board-state fallback, needs-triage enumeration, untriaged_orphans, needs_qa_numbers, grill candidate list, strict-blocker lookup, active_dev_orch, scout saturation, the arch/cleanup board read, wayfinder map list, tickets lane) | `orch_board_signals_degraded` (boolean, same shape as `orch_backfill_idle`) | **do-not-conclude-idle** (issue #4130): decide.py reads it pre-resolved (pure) and on `true` suppresses `terminate:idle`, the `_rule_idle_fallback` idle drain (heartbeat wait instead, `debug.idle_fallback="suppressed-degraded-board"`), and every `orch_backfill_idle`-keyed backfill class (`discover_orch`/`architecture_orch`/`cleanup_orch`/`skill_prune` — each records an `idle` dispatch_decision naming the outage). A GraphQL-only outage renders the whole board zero/none; without this flag that read as "no work" and idle-drained the run. Failed reads also stop emitting activating values (board-counts JSON, the four `target_*` counts, `orch_backfill_idle=true` — see the next row). |
| `target_board_signals_degraded=true` (the Target lane's aggregate: the board-state fallback read OR the direct Target board read failed this turn) | `target_board_signals_degraded` (advisory-merged boolean, issue #3478/#4130) | nothing gates on it directly — the degraded Target read already emits the suppressing defaults (`target_backfill_idle=false`, both caps saturated). Pre-#4130 the flag was the verdict of ONE call, so per-request flakiness could pair a degraded zero-set with `=false`; it is now the fold over the lane (issue #4130), and the four `target_*` counts are NOT emitted on a failed read — `target_ready_for_agent=0` was the board-empty condition that set `target_board_research_due` and phantom-dispatched research during the 2026-08-17 GraphQL-only outage. |
