...
| `orch_board_signals_degraded=true` (ANY orch board read FAILED — empty gh output, not an empty board; every fixed jq program prints at least `[]`/`{}` on success, so empty output ⟺ failure) | `state.signals.orch_board_signals_degraded` (boolean, merged like `orch_backfill_idle`) | gates decide.py (issue #4130, run 161d9642): while true, (a) NO `terminate` with an idle cause and no idle-drain backstop — a wait-only degraded turn emits a heartbeat wait with `debug.idle_fallback="degraded-board-hold"` instead (wall_clock/budget backstops stay honest); (b) the whole `orch_backfill_idle` backfill set (`discover_orch`, `architecture_orch`, `cleanup_orch`, `skill_prune`) is suppressed for the turn, each skip recorded as a `dispatch_decision` outcome `idle` naming the degradation. The same failure emits the `degraded` sentinel for the grill/dev anchor picks, which `_orch_anchor_signal` collapses like `none` (a failed read is not "no anchor"). Lane-scoped: does NOT touch the target lane. |
| `target_board_signals_degraded=true` (**target GH board** read failed — the target mirror of the orch flag, folded from the target board-state fallback since #4130; pre-#4130 the line was hardcoded `false` on the success path) | (advisory only) | nothing in decide.py today — the emission is the observable (#3478/#4130); wiring it into a target-lane hold is deliberately deferred (out of #4130's scope, which fixed the orch lane end-to-end) |
