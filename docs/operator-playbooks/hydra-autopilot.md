...
| `target_needs_triage > 0` (**target GH board**, scope=target — raw `needs-triage` label count; the #3059 blocker filter applies to `ready_for_agent` only) | `needs_triage_target` | `sweep_target` (issue #3709 — Target mirror of `needs_triage_orch`; no saturation cap, it drains the lane it gates on). Carries a **per-item verdict-stability guard** (issue #3729, AND-composed with this 900s cooldown) — see `target_needs_triage_items` below |
| `target_needs_triage_items=626,631,...` (**target GH board** — needs-triage item NUMBERS, sorted) | `state.signals.target_needs_triage_items` (verbatim string, no rename) | `sweep_target` per-item verdict-stability guard (issue #3729): suppress a sweep when EVERY current item was examined within a fixed 6h window (`state.target_triage_item_stamps`); stamp all current items on fire, prune the rest. This list derives from a SEPARATE board read than the `target_needs_triage` count, so a true count with an absent/empty list is a DEGRADED read → fail CLOSED (suppress), matching every sibling signal in the item-list read's block — never silently revert to guard-free (the #3872 fail-open defect) |
