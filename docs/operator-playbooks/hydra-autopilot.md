...
| `target_needs_triage_items=626,631,...` (**target GH board** — needs-triage item NUMBERS, sorted) | `state.signals.target_needs_triage_items` (verbatim string, no rename) | `sweep_target` per-item verdict-stability guard (issue #3729): suppress a sweep when EVERY current item was examined within a 6h window (`state.target_triage_item_stamps`); stamp all on fire, prune the rest. Absent/empty → fail-open (fire) |
