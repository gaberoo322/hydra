...
- `$TARGET_APP_DIR` — `$TARGET_WS` joined with `$TARGET_APP_SUBDIR` (empty subdir is the identity) — the directory the build/test/typecheck commands run from; never assume a hardcoded subdirectory nesting
...
`$TARGET_WS/docs/agents/domain.md`. **Each of these is optional** — not every
Target carries every doc (issue #4525). Check with `[ -f <path> ]` first and
skip a missing one explicitly; a missing optional doc is never an error and
never an input to invent.
