...
if [ -n "$REFL_SOURCES" ]; then
  if [ -z "$REFL_TASK_ID" ]; then
    ...
    printf '[hydra-dev] WARN refl-deposit-no-task-id: served %s but no harness task_id derivable from cwd=%s — reflectionMatchSource will read none\n' \
      "$REFL_SOURCES" "$PWD" >&2
  else
