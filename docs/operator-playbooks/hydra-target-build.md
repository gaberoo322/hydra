...
# `npm ci --prefer-offline` would error "command not found: npm ci --prefer-offline").
...
# eval word-splits multi-word manifest commands under zsh (bare `$TEST_CMD` is one token).
...
# eval word-splits multi-word manifest commands under zsh (bare `$TYPECHECK_CMD` is one token).
