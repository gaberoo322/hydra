...
Keep only ledger rows with status `wire-or-retire` (modules past the 45-day grace with no runtime importer — `awaiting-wiring` and `protected-provider` rows are never eligible), dedup per module against open `wire-or-retire`-labelled issues (lexical `gh issue list --search` on the Target repo), cap at **3 per run** (oldest last-touched first; saturation back-stop at 5 open items), and file each via `gh issue create --repo gaberoo322/hydra-betting --label needs-triage --label wire-or-retire`, rendering title + body from the same row.

> The `scripts/ci/hydra-target-wire-or-retire-emit.ts` runner that previously bundled this step behind a `/api/backlog` call was dead code — every route it hit (`GET`/`POST`/`PATCH` `/api/backlog`) retired with `src/api/backlog.ts` in PR #3455, so it threw `404` before evaluating a single ledger row — and is removed in #3747. This `gh issue create` path never touches `/api/backlog` and is the sole wire-or-retire producer.
