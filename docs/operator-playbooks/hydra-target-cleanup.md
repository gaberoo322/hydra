...
After the demote emit, process the judgment half directly against the Target's
**committed wiring-status ledger** (`docs/agents/wiring-status.md`, generated
Target-side by `npm run deadcode:ledger` — hydra-betting PR #98). Never
regenerate the ledger from this skill: the scan must not mutate the Target's
working tree; staleness is handled by each item's verify-first step plus
dedup. There is no emit script for this phase (issue #3747 removed
`scripts/ci/hydra-target-wire-or-retire-emit.ts` — it POSTed to the retired
`/api/backlog` HTTP surface and had zero production callers; every real
wire-or-retire item has always been filed by an agent following THIS prose
directly) — the agent running `/hydra-target-cleanup` performs the steps
below itself, with `gh` as the only tool:

1. **Read the ledger**: rows with status `wire-or-retire` are eligible
   (modules past the 45-day grace with no runtime importer —
   `awaiting-wiring` and `protected-provider` rows are never eligible).
2. **Dedup**: `gh issue list --repo gaberoo322/hydra-betting --label wire-or-retire --json title` —
   drop any ledger row whose module path already appears in an open
   `wire-or-retire`-labelled issue title.
3. **Cap**: at most **3** per run, oldest `lastTouched` first; if the open
   `wire-or-retire` count already exceeds **5** (the saturation back-stop),
   file nothing this run.
4. **File** each surviving row via
   `gh issue create --repo gaberoo322/hydra-betting --label needs-triage --label wire-or-retire`,
   with title `cleanup(target): wire-or-retire <module path>` and a body
   carrying the three-way decision protocol below — title and body must come
   from the same ledger row in one pass (the #1449/#1005 drift guard).
...
**Pairing invariant (this is additive, not a swap).** This emit path is the only place `wire-or-retire` is ever added, and it is always added *together with* `needs-triage` in the same `gh issue create` call above — never onto an item that lacks `needs-triage`, and never as a lane of its own. Once filed, the pair is inert to every board writer except `hydra-wire-or-retire`: `hydra-target-sweep`'s triage step explicitly exempts any `wire-or-retire`-carrying item from its auto-promote test rather than stripping `needs-triage` off it (`docs/operator-playbooks/hydra-target-sweep.md` Step 2), and `hydra-wire-or-retire`'s own work-finding query requires co-presence of both labels (`docs/operator-playbooks/hydra-wire-or-retire.md`), so only this resolver can retire the pairing.
...
- The judgment phase (wire-or-retire) has **no emit script** — issue #3747 removed `scripts/ci/hydra-target-wire-or-retire-emit.ts` (POSTed to the retired `/api/backlog`, zero production callers) and `test/hydra-target-wire-or-retire-emit.test.mts` with it; step 3.5 above is the sole, agent-followed producer via direct `gh` calls.
