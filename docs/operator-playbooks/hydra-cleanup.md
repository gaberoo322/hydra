...
- **It touches the Verifier Core** (`src/untouchable.ts` `VERIFIER_CORE_PATHS`: `.github/workflows/ci.yml`, `.github/workflows/deploy.yml`, `.github/workflows/deep-qa-gate.yml`, `scripts/tier-classify.ts`, `src/tier-classifier.ts`, `src/untouchable.ts`). Those are operator-only (ADR-0001/0004/0015/0020). Never steer an agent at them. Note the finding in the report, but do not file it.
...
**Labelling rule (HARD):** every emitted issue carries `cleanup-scan`, `ready-for-agent`, and `design-concept-exempt`. The `cleanup-scan` label is the emit/count seam that `collect-state.sh` reads for `cleanup_board_saturated`, so it MUST be present on every issue — and it is now also the **load-bearing exemption key** read by `hydra-qa`'s parent-issue exempt check, `collect-state.sh`'s grill gate, and the GLM drainer's grill-clear predicate (issue #4431): `cleanup-scan` issues filed outside this emit runner (a `cleanup_orch` session filing by hand, or a sweep re-filing) have historically skipped the `design-concept-exempt` label the playbook marks HARD, so keying the consumers on `cleanup-scan` instead makes the exemption robust to every filing path, not just this one. Routing to `ready-for-agent` (NOT `needs-triage`) is the deliberate confidence-routing decision (epic #958): the acceptance criterion is self-checking, so no operator triage gate is needed — a `hydra-dev` pickup will only merge if the deletion keeps `npm test` and `tsc` green, and CI is the merge gate. The `design-concept-exempt` label is still required on every emitted issue and is **kept for back-compat** (other readers, e.g. the design-concept-comment.yml PR-label allowlist, still key on it) — without either label, `hydra-qa` logs a QA resolve MISS and falls through to Phase A shadow mode, silently dropping the Spec axis verdict rather than explicitly bypassing it (issue #3013).
...
Expected (mechanisms are detailed in the Process steps above; this is the checklist, not a re-derivation): fresh-base fetch before scan, aborting on a failed fetch or a non-fast-forwardable worktree (#1318); `knip` runs and the report parses into files vs exports; the filter drops verifier-core, test-only, entrypoint, and duplicate findings; `--apply` files issues labelled `cleanup-scan` + `ready-for-agent` + `design-concept-exempt`; title and body never drift across a multi-finding batch because both render from the same finding in one pass (#1005, #1449); every issue carries a `## Files in scope` section (#1077); a saturated board (> 10 open `cleanup-scan`) emits nothing; re-running never double-files an identity already open as a legacy title or a batch `cleanup-identities` manifest (#1653); export findings behind a namespace-import / DI-facade are suppressed with an audited drop reason, file-kind findings unaffected (#1737); findings covered by an open or recently-merged PR are suppressed with the covering PR number, and a failed PR fetch or a knip report older than 60 minutes aborts the emit rather than degrading silently (#1766); a finding with an empty name/path is dropped before render, so a blank-title issue is impossible (#1167). Pinned in `test/hydra-cleanup-emit.test.mts` + `test/hydra-cleanup-render.test.mts`.
...
- `scripts/ci/hydra-cleanup-emit.ts` — the deterministic emit runner (issues #1449, #1653, #1737, #1766): `planCleanupEmit()` (pure: parse → validate → filter → classify → dedup → group-by-module → render) plus the thin `gh` CLI wrapper. Replaces the hand-rolled bash emit loop that regressed #1167; batches per module dir since #1653 (`SYMBOLS_PER_BATCH = 20`, `EMIT_CAP` counts batch issues); since #1737 the wrapper injects the namespace-import consumer set (`collectNamespaceConsumedModules()` + pure `resolveNamespaceImportTargets()`) so the step-1 filter drops the DI-facade export false positives with an audit reason; since #1766 the wrapper injects the covering-PR surface (`readCoveringPrs()` via REST + pure `filterPrsInDedupWindow()`, `MERGED_PR_DEDUP_WINDOW_MS = 24h`), aborts on a failed PR fetch, and refuses a knip report older than `KNIP_REPORT_MAX_AGE_MS` (60 min) via the shared loader in `scripts/ci/hydra-knip-source.ts` (issue #4523).
- `scripts/ci/hydra-knip-source.ts` — the shared knip-report source loader (issue #4523): `loadKnipReport()` (exists → staleness → parse, Result-shaped, never throws) + `KNIP_REPORT_MAX_AGE_MS`. Both `hydra-cleanup-emit.ts` and `scripts/ci/hydra-target-cleanup-emit.ts` wire `spec.loadSource` to this one function instead of hand-declaring their own, so the #1766 staleness guard is lane-symmetric.
