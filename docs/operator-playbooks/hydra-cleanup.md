...
- `--apply` files issues labelled `cleanup-scan` + `ready-for-agent` + `design-concept-exempt` — each with the deterministic "remove X AND test/tsc green" acceptance criterion.
