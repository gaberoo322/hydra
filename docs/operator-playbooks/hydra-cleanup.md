...
- **Every issue carries `cleanup-scan` + `ready-for-agent` + `design-concept-exempt`.** The `cleanup-scan` label is the count seam for `cleanup_board_saturated`; the routing is the confidence decision; the exempt label tells `hydra-qa` to skip the Spec axis without a MISS warning (issue #3013).
