...
- On a `wayfinder:map` issue. Maps look epic-shaped (they carry a `## Sub-issues` block by design) but their death is owned by `/hydra-wayfinder`'s own handoff step: a cleared map must survive until the operator hands it off to `hydra-prd` / `/to-spec`, then `/hydra-wayfinder` closes it (or sets `keep-open`). (Issue #4179 deleted `/hydra-review`'s parallel §0.7 handoff bucket, which had never fired; the exclusion below is UNCHANGED — only the name of the surface that owns the handoff moved.) Step 1c drops every `wayfinder:map` from the candidate scan so this sweeper never closes one out from under a pending handoff.
...
#     maps are closed by /hydra-wayfinder's own handoff step, never this sweeper,
#     so a cleared map is never closed before the operator can hand it off.
#     (Issue #4179 retired /hydra-review's parallel §0.7 handoff bucket; this
#     exclusion is unchanged -- only the owning surface's name moved.)
jq -s '
  (.[0] + .[1])
  | unique_by(.number)
