...
<concrete paths under ~/hydra that this deepening touches — as a bullet
list, ONE repo-relative path per line; see the scope-format rule below>
...
candidate must not touch — as a bullet list, ONE repo-relative path per
line; see the scope-format rule below>
...
<concrete paths under ~/hydra this idea touches — as a bullet list, ONE
repo-relative path per line (scope-format rule in the 4.1 emission section);
promotion to ready-for-agent later requires this section to parse non-empty>
```
