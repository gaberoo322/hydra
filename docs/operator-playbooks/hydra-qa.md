...
> **Composed skill (ADR-0030 Decision 4 / Option C, issue #3420).** This playbook is the thin Hydra **AFK overlay** on top of the vendored upstream `code-review` base (`docs/operator-playbooks/_vendor/code-review.md`). `scripts/sync-skills.sh` emits `~/.claude/skills/hydra-qa/SKILL.md` as **[upstream code-review base] + [this overlay]**, with the vendored base's `disable-model-invocation: true` **stripped** (it hard-errors under Skill-tool dispatch). This is the keystone tracer for the one-lineage spine: the review stage dispatches the *same* upstream `code-review` skill the operator runs, in AFK mode. The Hydra-specific verification depth, verdict classification, and remediation-loop routing below ride on that shared base. The **review** stage's binding is one of the three the one-lineage refit rebound (ADR-0030 Decision 2): alpha (#3420) rebound review here, gamma (#3422) completed the set by rebinding **implement** (`hydra-dev` → `_vendor/implement.md`) and folding the **spec** stage's grill-before-build gate in over `_vendor/to-spec.md`, and epsilon (#3424) contracted the sequence — see below; the dispatch-class → stage table lives in `hydra-autopilot.md`. **Contract complete (ADR-0030 Decision 5, epsilon #3424):** the standalone `hydra-qa` *fork identity* is retired as a documented concept — it is no longer a bespoke reviewer fork, it **is** the composed `review` stage. The `qa_orch` dispatch *class* and its `decide.py` `make_dispatch(…, "hydra-qa")` string literals (orch + target scope) stay live (they select this composed stage; delta #3423 already migrated the learning-loop seams), but "hydra-qa the fork" no longer names a second inline copy of the review pattern.
