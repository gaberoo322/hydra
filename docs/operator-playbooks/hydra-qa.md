...
> **Composed skill (ADR-0030 Decision 4 / Option C, issue #3420).** This playbook is the thin Hydra **AFK overlay** on top of the vendored upstream `code-review` base (`docs/operator-playbooks/_vendor/code-review.md`). `scripts/sync-skills.sh` emits `~/.claude/skills/hydra-qa/SKILL.md` as **[upstream code-review base] + [this overlay]**, with the vendored base's `disable-model-invocation: true` **stripped** (it hard-errors under Skill-tool dispatch). This is the keystone tracer for the one-lineage refit: the review stage now dispatches the *same* upstream `code-review` skill the operator runs, in AFK mode. The Hydra-specific verification depth, verdict classification, and remediation-loop routing below ride on that shared base. The **review** stage's binding is one of the three the one-lineage refit rebinds (ADR-0030 Decision 2, expand step): alpha (#3420) rebound review here, and gamma (#3422) completes the set by rebinding **implement** (`hydra-dev` → `_vendor/implement.md`) and folding the **spec** stage's grill-before-build gate in over `_vendor/to-spec.md`; the dispatch-class → stage table lives in `hydra-autopilot.md`.
