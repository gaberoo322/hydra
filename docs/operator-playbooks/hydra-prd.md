...
> **Demoted to a called renderer library (ADR-0030 Decision 2, epsilon #3424).** `hydra-prd` is **no longer a standalone dispatch identity** in the one-lineage Pocock spine (`plan` → `spec` → `tickets` → `implement` → `review`). The **tickets** stage is owned by the `tickets_orch` dispatch class (beta #3421, delta #3448) composed on the vendored upstream `to-tickets` base; that overlay **calls** this skill's pure `PrdInput`→GitHub-issue renderer (`scripts/ci/hydra-prd-render.ts`) as a **bridge library**. `hydra-prd` never had a `decide.py` `make_dispatch(…)` class row of its own (it was operator/sub-dispatched), so its retirement is an **identity/framing** change — the renderer functions (`renderParentBody` / `renderChildBody` / `parentLabels` / `childLabels`) stay live and are unit-tested (`test/hydra-prd-template.test.mts`); autopilot dispatches the `tickets_orch` row, **never** `hydra-prd`. This playbook documents the renderer's input/output contract; it is not a class the autopilot fires.
