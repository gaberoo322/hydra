...
## Target worktree lifecycle (issue #2115 / #2465 / #3136 / #4177)

A code-writing dispatch into the Target (`hydra-target-build`) runs in a fresh worktree under `<targetWorkspace>/web/.worktrees/` (e.g. `~/hydra-betting/web/.worktrees/`), on a branch named `feature/claude-cycle-<timestamp>`. On a **clean** finish the harness removes the worktree and the branch is deleted. When a dispatch **crashes or exits uncleanly**, the worktree dir *and* its `.git/worktrees/<id>` registry entry survive with the feature branch still checked out — and because git refuses `git branch -D` on a branch a registered worktree still holds, that stale branch cannot be reclaimed until its worktree is removed.

> **Relocated 2026-08-26 (issue #4177, prevention half of #4175).** Before this, the worktree lived under `/dev/shm/hydra-worktrees/` (RAM-backed, fast, disappears on reboot) — but a `/dev/shm` worktree has no ancestor `node_modules`, so it needed either a slow per-worktree `npm ci` or a symlink straight back at the main checkout's real `web/node_modules`. An agent running a destructive install through that symlink took down six money-critical Target services for ~70 minutes (2026-08-19 incident). `web/.worktrees/<name>` sits two levels under `web/` itself, so Node's upward directory walk finds the main checkout's real `node_modules` directly: no per-worktree install, no symlink, and a destructive `rm -rf node_modules` inside the worktree only ever touches a worktree-local path. The trade-off is disk instead of tmpfs for the worktree's own checked-out files — accepted deliberately, safety over I/O speed.
...
The hourly chore (issue #3136) is the backstop for orphans created *between* restarts — a mid-cycle crash whose worktree only crosses the 6h age floor hours later would otherwise survive until the next boot and fail the branch-cleanup path on every tick. Both paths apply the **same never-touch-first liveness discipline**: a worktree is reclaimed ONLY when it is not the main tree, not the current branch, inside the `<targetWorkspace>/web/.worktrees/` scope prefix (derived from the configured target workspace, not a hardcoded host path), not held by a **live** agent PID (`kill -0` probe; an invalid/unknown PID is treated as live), and past the **6h age floor** (`DEFAULT_WORKTREE_MIN_AGE_SECONDS`) — so an in-flight dispatch that has not yet written its lock file is never destroyed. The reclaim is idempotent (a second run over an already-clean set is a no-op) and never-throwing. Triggered cleanups surface on the `[Housekeeping]`/`[Hydra]` log channel (a clean tick is silent). Worktrees **outside** the `<targetWorkspace>/web/.worktrees/` prefix are deliberately left for the broader `hydra-branch-prune` skill's multi-pass GC.
...
| **T4 — Verifier Core** | **Operator only** — `operator-approved` label required; CI blocks otherwise | The 6 self-referential files (`VERIFIER_CORE_PATHS` in `src/untouchable.ts`) |

**Multi-file PRs:** the highest tier wins (most verification / scrutiny).

**Verifier Core list (`VERIFIER_CORE_PATHS` / `isVerifierCore` in `src/untouchable.ts`):** `.github/workflows/ci.yml`, `.github/workflows/deploy.yml`, `.github/workflows/deep-qa-gate.yml`, `scripts/tier-classify.ts`, `src/tier-classifier.ts`, `src/untouchable.ts`. ADR-0015 (#737) shrank the deepest tier to exactly 5 self-referential files and demoted the former Tier-0 infra paths (`src/grounding.ts`, `src/cost/`, the watchdog scripts, `scripts/deploy.sh`) down to T3; ADR-0020 (#847) then added a 6th self-referential file, `deep-qa-gate.yml` (the required check that verifies the SHA-bound Deep-QA PASS marker), bringing the list to 6. The pre-#383 entries `src/gate.ts` / `src/verification.ts` / `src/post-merge.ts` / `src/control-loop.ts` / `src/redis-adapter.ts` no longer exist (removed with the codex cut-over). "Untouchable Core" is the retired name for the Verifier Core.
...
**Subagent session-id capture hooks (issue #692 + #2406):** the subagent-dispatch registry `hydra:dispatches:subagent:{sessionId}` (24h TTL, indexed at `hydra:dispatches:subagent:index`) is populated by **two** triggers that both scrape the hidden `<!-- hydra-dispatch v1 skill=… dispatchId=… runId=… -->` sentinel from a session's transcript and POST it to `POST /api/dispatches/subagent`, making a live session recoverable to `(skill, dispatchId, runId, startedAt)`. Both share one grammar source-of-truth, `scripts/hooks/extract-dispatch-sentinel.sh` (sourced, not executed):

- **`SessionStart` hook** (`scripts/hooks/session-start-capture.sh`, registered in the project-scoped `~/hydra/.claude/settings.json`) — fires for **top-level** sessions (the operator running `claude`, or the autopilot parent). No install step; Claude Code loads project settings automatically inside `~/hydra`.
- **`PostToolUse` hook** (`scripts/autopilot/hooks/on-subagent-tool-call.sh`, registered per-skill via the sibling `<skill>.settings.json` that `sync-skills.sh` syncs) — fires **inside** Agent-tool subagent children, which `SessionStart` structurally never reaches (#2406 root cause: the registry was empty for every Agent-tool dispatch). It registers on the first tool call, guarded by a once-per-session marker under `${HYDRA_DISPATCH_REGISTER_MARKER_DIR:-/tmp}` so the scrape+POST runs once per session (the registry write is idempotent regardless). This trigger is decoupled from the offline usage-attribution work (#2401/#2402): they share only the sentinel grammar, never a mechanism, and the registration introduces no registry dependency into `src/cost/*`.

Sessions without the sentinel (a human running `claude`) silently no-op. Both hooks are best-effort: a Redis/HTTP outage, missing sentinel, missing transcript, or missing `jq` logs to stderr and exits 0 without ever blocking the session or the tool call; re-registering the same session is an idempotent no-op. Smoke-test: `redis-cli zrange hydra:dispatches:subagent:index 0 -1 WITHSCORES` should show a fresh entry within ~5s of the autopilot dispatching a `hydra-dev` subagent.
