...
Related: #3426 (wayfinder map — the decision + destination), #3427 (research — GitHub-Issues rate-limit viability for a machine-driven loop), #3428 (research — consequence-map of dropping the Redis scoring/dedup/suppression engine), #3429 (the MIGRATE keystone decision + accepted costs), #3432 (the spec this ADR governs), ADR-0002 (betting target is the swappable single-target crucible), ADR-0029 (autopilot charts and works wayfinder maps — the map machinery this used), #3059 (strict blocked-by/depends-on board filter the Target inherits), ADR-0004 / ADR-0015 (self-modification tiers), #3700 (Decision 5 amendment — the merged-PR issue-close is explicit, not native)
...
- **Merged / shipped-subject suppression** (#882 / #3208) → **enforced `Closes #N` close-discipline** in Target `hydra-dev`, so a merged PR closes its issue and leaves the open board clean. The positive-evidence merged-blob matchers (`merged-refs` / `token-algebra`, already repo-swap-aware) are retained as an **optional reconciler sweep**, not a hot-path gate.

  > **Amended 2026-07-25 (#3700) — the close is explicit, not free.** As originally written this bullet said a merged PR *auto*-closes its issue "for free", assuming GitHub's native `Closes #N` handling. **That premise is false on this repo.** GitHub performs the linked-issue close *as the merging actor*, and the Target's emulated automerge (`hydra-betting/.github/workflows/automerge.yml`, standing in for the branch protection a free-tier private repo lacks) merges as `GITHUB_TOKEN` under an explicit `permissions:` block — which sets every unlisted scope to `none`, so the token held `issues: none` and the close was skipped silently. Measured: **0 of ~50 `github-actions[bot]` merges auto-closed**, against 5 of 5 direct operator merges closing in ≤1 second. The closing *link* was never lost (GraphQL `closingIssuesReferences` still resolves it); only the close *action* was dropped, leaving issues OPEN and still labelled `ready-for-agent` — which re-dispatched already-shipped work.
  >
  > Close-discipline therefore stands as the decision, but it is **executed explicitly**: `automerge.yml` holds `issues: write` and, immediately after merging, resolves the PR's `closingIssuesReferences` and closes each linked issue as `completed`, stripping the board labels. Eligibility keys on GitHub's own `merged` boolean, so a PR closed without merging can never close an issue. The reconciler sweep the bullet already anticipated is correspondingly **not optional in practice** — `hydra-target-sweep` Step 5's stall-recovery must branch on merged-PR state (it previously queried only `state=open`, which cannot see a merged PR, and so re-armed shipped issues) and Step 7 remains the title-overlap backstop. The `hydra-target-build` merge-flow close is retained as a residual guard, demoted from the mechanism to belt-and-suspenders.
  >
  > Left standing deliberately: whether `issues: write` *alone* would restore GitHub's native auto-close is unconfirmable without a live merge experiment, and the design is independent of the answer — the permission is a precondition of the explicit close, never an alternative to it. Switching the merge to a PAT or GitHub App token would also fix it, but that is a credentials change (ADR-0005 operator-escalation territory) on the Target's only merge path, and is recorded as a follow-up rather than folded in.
