/**
 * Regression tests for the /api/v2/now/* route handlers (issue #618).
 *
...
} from "../src/schemas/v2/now.ts";
import { createV2NowRouter, parseAlertsWindow } from "../src/api/v2/now.ts";
...
describe("GET /v2/now/autopilot-tick", () => {
  test("happy path — both sub-sources fulfilled", async () => {
    ...
    const handler = findHandler(router, "GET", "/v2/now/autopilot-tick");
    assert.ok(handler);
    const res = mockRes();
    await handler!(mockReq(), res);
    assert.equal(res._status, 200);
...
describe("GET /v2/now/active-dispatches", () => {
  test("happy path — aggregator stub plumbs through", async () => {
    ...
    const handler = findHandler(router, "GET", "/v2/now/active-dispatches");
    assert.ok(handler);
    const res = mockRes();
    await handler!(mockReq(), res);
    assert.equal(res._status, 200);
...
describe("GET /v2/now/cost-burn", () => {
  test("happy path — stamps generatedAt and passes through aggregator fields", async () => {
    ...
    const handler = findHandler(router, "GET", "/v2/now/cost-burn");
    assert.ok(handler);
    const res = mockRes();
    await handler!(mockReq(), res);
    assert.equal(res._status, 200);
...
describe("GET /v2/now/alerts", () => {
  test("happy path — filters by sinceMinutes window", async () => {
    ...
    const handler = findHandler(router, "GET", "/v2/now/alerts");
    assert.ok(handler);
    const res = mockRes();
