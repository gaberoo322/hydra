/**
 * Regression tests for GET /api/v2/today/summary (issue #616, PRD #615).
 *
 * Pins:
...
} from "../src/schemas/v2/today.ts";
import { createV2TodayRouter } from "../src/api/v2/today.ts";
...
function mockReq(query: Record<string, unknown> = {}): any {
  return { method: "GET", url: "/v2/today/summary", headers: {}, query, params: {}, body: {} };
}
...
describe("GET /v2/today/summary — route handler", () => {
  test("happy path: aggregator stub plumbs through unchanged", async () => {
    ...
    const router = createV2TodayRouter({
      getOvernightSummary: async (windowHours) => {
        observedWindow = windowHours;
        return stubSummary;
      },
    });
    const handler = findHandler(router, "GET", "/v2/today/summary");
    assert.ok(handler);
