...
/**
 * Shared LIVE sync (issue #4502). A full `sync-skills.sh` run over the REAL
 * playbook set takes ~11s (it spawns a dozen python3 passes per playbook), and
 * this file used to regenerate it from scratch in ~13 separate cases — ~145s of
 * the file's ~170s wall time. Every "live" golden check below only READS the
 * generated bytes, and the generator is deterministic over an unchanged
 * playbook tree, so one run is the honest unit of work: it is generated lazily
 * on first use, reused by every read-only live case, and removed once the file
 * finishes. A case that must MUTATE generated output (the #3995 idempotency
 * check) works on a private copy, never on this shared tree.
 */
let liveSyncResult:
  | { status: number | null; stdout: string; stderr: string; dir: string; claudeDir: string; codexDir: string }
  | undefined;
function liveSync(): NonNullable<typeof liveSyncResult> {
  if (liveSyncResult) return liveSyncResult;
  const dir = mkdtempSync(join(tmpdir(), "sync-skills-live-shared-"));
  ...
  liveSyncResult = { status: r.status, stdout: r.stdout, stderr: r.stderr, dir, claudeDir, codexDir };
  return liveSyncResult;
}
after(() => {
  if (liveSyncResult) rmSync(liveSyncResult.dir, { recursive: true, force: true });
});
...
describe("scripts/sync-skills.sh — playbook edit propagates to generated SKILL.md (issue #433)", () => {
  test("editing a playbook regenerates the matching skill with the new content", () => {
    ...
    //
    // Issue #4502: the redirected sync output is the file-wide shared live
    // sync (read-only here), not a per-case regeneration.
    {
      assert.ok(existsSync(playbookSrc), "hydra-dev playbook must exist as the test fixture");
      // Run sync with redirected output dirs.
      const r = liveSync();
      const claudeDir = r.claudeDir;
...
describe("live hydra-qa — exactly one live instruction survives the compose seam (issue #3991)", () => {
  ...
  function syncLive(): string {
    // Issue #4502: read from the file-wide shared live sync (read-only).
    const r = liveSync();
    assert.equal(r.status, 0, `live sync failed: ${r.stderr}`);
    return readFileSync(join(r.claudeDir, "hydra-qa", "SKILL.md"), "utf-8");
  }
...
describe("live hydra-architecture-scan — the hand-copied upstream steps are composed, not restated (issue #3993)", () => {
  ...
  // (and since #4502 that one sync is the file-wide shared live sync).
  function syncLive(): string {
    const r = liveSync();
    assert.equal(r.status, 0, `live sync failed: ${r.stderr}`);
    return readFileSync(
      join(r.claudeDir, "hydra-architecture-scan", "SKILL.md"),
      "utf-8",
    );
  }
...
describe("live thermo-nuclear-code-quality-review + zoom-out — ungoverned skills promoted to tracked, gated playbooks (issue #3995)", () => {
  ...
  test("regeneration is idempotent — a second sync-skills.sh run produces no diff (#3995 AC)", () => {
    // Issue #4502: the FIRST run is the file-wide shared live sync. Its output
    // is copied into a private dir (the shared tree stays read-only), and the
    // SECOND run regenerates over that copy — exactly "run sync twice into the
    // same destination", at the cost of one full sync instead of two.
