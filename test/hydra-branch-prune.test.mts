 ...
 * — pass 1 ([gone]-upstream classification) plus the shared parsing helpers.
 *
 ...
 *
 * Split from a single file into five siblings by classifier pass (issue
 * #4062) to shrink the per-file top-level-entry count that drives the
 * `--test-force-exit` suite-count-drop race — see
 * test/_helpers/branch-prune-fixtures.mts's header for the measured
 * evidence. This file (pass 1 + shared parsers) is sibling 1 of 5:
 * test/hydra-branch-prune-worktree-orphan.test.mts (issue #911),
 * test/hydra-branch-prune-dead-branch.test.mts (issue #1784 / #2115),
 * test/hydra-branch-prune-merged-remote.test.mts (issue #2029),
 * test/hydra-branch-prune-master-tracking.test.mts (issue #2459).
 */
...
import { branch, wt, NEVER_LIVE, ALWAYS_LIVE, OLD, YOUNG } from "./_helpers/branch-prune-fixtures.mts";
