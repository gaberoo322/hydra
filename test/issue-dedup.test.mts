/**
 * Regression tests for the shared issue-dedup baseline used by the
 * Orchestrator board-filling skills — primarily `hydra-discover` and
 * `hydra-architecture-scan` (the two that co-fire on `orch_backfill_idle`),
 * plus `hydra-research` (operator-only) — to decide whether a candidate
 * finding duplicates an already-tracked issue (issue #2554).
 *
 * The rule itself is documented in the Dedup sections of
 * `docs/operator-playbooks/hydra-discover.md`,
 * `docs/operator-playbooks/hydra-architecture-scan.md`, and
 * `docs/operator-playbooks/hydra-research.md`, and implemented in
 * `scripts/ci/issue-dedup.ts`. Before this helper, each playbook eyeballed
 * ">50% title overlap" independently AND only against its own label set, so
 * the discover<->architecture-scan pair firing in the same idle window could
 * disagree at the margin and double-file the same underlying gap. These tests
 * pin the deterministic judgement the skills now share:
 *
...
describe("issue-dedup: isDuplicateIssue", () => {
  ...
  test("a discover finding duplicates an architecture-scan issue for the same gap (cross-class baseline)", () => {
    // The motivating case: discover and architecture-scan co-fire on
    // orch_backfill_idle. A discover candidate must dedup against an
    // architecture-scan issue already on the board for the same underlying gap.
    const sharedBaseline = [
      "architecture-scan: deepen the anchor-selection module behind an interface",
    ];
    const verdict = isDuplicateIssue(
      "Deepen anchor-selection module behind an interface",
      sharedBaseline,
    );
    assert.equal(verdict.duplicate, true);
    assert.equal(verdict.matchedTitle, sharedBaseline[0]);
  });
...
describe("issue-dedup: partitionCandidates", () => {
  ...
  test("a candidate that duplicates an earlier sibling is also caught when seeded into the baseline", () => {
    // Cross-skill double-file guard: discover files A, then architecture-scan
    // (with A now in its baseline) must skip a re-phrasing of A.
