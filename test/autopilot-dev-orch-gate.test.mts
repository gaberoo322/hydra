...
describe("hydra-autopilot dev_orch rule (issue #412)", () => {
  ...
  test("decide.py gates dev_orch on the live PR signal, not the in-progress label", () => {
    // Find the dev_orch handler registered in _SLOT_SELECTORS (issue #4265:
    // the per-class dispatch god-function was split into one handler per
    // class, keyed by a registry, rather than a bare `if cls == "dev_orch"`
    // chain).
    assert.match(decide, /"dev_orch":\s*_select_slot_dev_orch/);
...
describe("decide.py — dev_orch route_model frontier hint on a pinned anchor (issue #3798)", () => {
  ...
  test("decision core does not consult the retired candidate-feed design-concept path (#751, #3455)", () => {
    ...
    // god-function was extracted into its own module-level handler.
