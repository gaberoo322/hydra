 ...
 * in 28h, #626 took 12 in 36h), plus a third shape: an operator-decision item
 * (betting#758) re-firing an identical no-op verdict every cooldown with no
 * future self-clear date. Options A (date-gating in collect-state.sh) and C (a
 * parked-until label) are explicitly rejected: A suppresses the very dispatches
 * that promote items out of the lane, and C reintroduces the #3720
 ...
 *     prunes the rest.
 *
 * FAIL-CLOSED COUPLING (the QA fix for the failed first attempt, PR #3872): the
 * count read (`needs_triage_target`, from collect-state.sh's scope=target
 * board-state endpoint) and the item-list read (`target_needs_triage_items`,
 * from the separate wire-or-retire/cleanup `gh issue list` scan) are TWO
 * INDEPENDENT board reads. A transient failure of the item-list read leaves the
 * count true but the list absent/empty. The guard MUST fail CLOSED in that state
 * (suppress the dispatch, matching every sibling signal in the item-list read's
 * block — target_cleanup_board_saturated / wire_or_retire_target_available /
 * design_qa_target_due all suppress on the identical degraded read), never
 * silently revert to guard-free. The prior attempt fail-OPENED here and that
 * asymmetry is exactly what its QA rejected.
 *
...
function makeTmp(): Tmp {
  const dir = mkdtempSync(join(tmpdir(), "decide-sweep-target-signal-"));
  return {
    dir,
...
describe("decide.py — sweep_target per-item verdict-stability guard (issue #3729)", () => {
  ...
  test("fail-CLOSED: a true count with an ABSENT item list suppresses (issue #3872 QA fix)", () => {
    // needs_triage_target is true (the count read succeeded) but collect-state.sh
    // emitted no item list (the separate item-list read degraded). This is the
    // DEGRADED state PR #3872 fail-opened on (silently firing, guard-free,
    // thrash-prone). The fix is fail CLOSED: suppress, matching every sibling
    // signal in the item-list read's block. This is the asymmetry the #3872 QA
    // rejected, inverted.
    ...
    assert.equal(
      findAction(plan, sweepTarget),
      undefined,
      "a true count with no enumerable item list must fail closed (suppress), not silently revert to guard-free",
    );
  });

  test("fail-CLOSED: a true count with an EMPTY item-list string also suppresses", () => {
    // A present-but-empty list ("needs_triage_target true but items=''") is the
    // degraded item-list read too (the count read found items the item-list read
    // could not enumerate) -> fail closed, identical to the absent case.
    const state = baseState({
    ...
    assert.equal(
      findAction(plan, sweepTarget),
      undefined,
      "a true count with an empty item list must fail closed, not fire",
    );
  });
...
describe("collect-state.sh — target_needs_triage_items emission (issue #3729)", () => {
  ...
  test("a triage-empty board emits an empty list", () => {
    // A board this read found to have zero needs-triage items. decide.py never
    // reaches the guard here because the separate count read also returns 0
    // (needs_triage_target false); the empty list is the honest "zero items"
    // value, not a degraded sentinel.
