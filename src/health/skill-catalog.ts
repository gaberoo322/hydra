...
// FAILURE RATE of the last completed pass.
//
...
// — and fires when it crosses `REGISTRATION_FAILURE_RATE_THRESHOLD`, pointing at
// OpenViking load as the cause.
//
// Issue #3544: the alert once correlated this failure rate with a Tailnet Ollama
// VLM-host liveness facet (#2284) to distinguish a VLM-offline cascade from an
// OV-load failure. At the VLM cutover, OpenViking's VLM backend moved off the
// gaming-PC host onto the in-repo claude-cli shim (#3542), so that liveness facet
// no longer measures anything OpenViking depends on — the alert now reports the
// single remaining cause (OpenViking overloaded/5xx-ing under indexing load).
//
// This is ADDITIVE and strictly read-only: it consumes the existing
// `SkillCatalogSnapshot`, owns no new export on skill-registration.ts, and never
// mutates the catalog state. It is distinct from `assessSkillCatalog`'s
// empty/partial verdict — an `empty` catalog (rate 100%) ALSO trips this alert,
// but with the failure-rate + recovery framing the population gate omits, so an
// operator sees both "catalog is empty" and "and here is why + how to recover".
// It fires `warning` (not `error`) so it never escalates the deep-health fold
// above the population gate's own severity — it annotates, it does not outrank.
// ---------------------------------------------------------------------------
 ...
 *
 * Reads ONLY its `snap` argument — the catalog snapshot — and never throws
 * (src/health convention). It does NOT mutate skill registration state and adds
 * no new export to skill-registration.ts; it is a pure consumer of
 ...
 *
 * Issue #3544: the alert once took a Tailnet Ollama VLM-host liveness facet
 * (#2284) to distinguish a VLM-offline cascade from an OV-load failure. At the
 * VLM cutover the gaming-PC VLM host stopped serving OpenViking (#3542), so the
 * alert now reports the single remaining cause — OpenViking overloaded under
 * indexing load — and points at the ollama-recovery operator playbook.
 */
export function assessRegistrationFailureRate(
  ...
  // warning) with the failure-rate framing. It must not ESCALATE the top-level
  // deep-health status above what the population gate decided — a partial catalog
  // stays `degraded`, an empty one stays `unhealthy` from the population rule's
  // own `error`. So this rule contributes a same-or-lower severity finding that
  // adds the "why + how to recover" detail without changing the fold.
  return {
    severity: "warning",
    component: "intelligence",
    what: `OV skill registration failure rate ${pct}% (${failed}/${snap.total} failed)`,
    why: "The last skill-registration pass failed above the 10% alert threshold — OpenViking is likely overloaded/5xx-ing under indexing load (#1924/#1831).",
    impact:
