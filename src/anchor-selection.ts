...
const STUCKNESS_COOLDOWN_PREFIX = "hydra:stuckness:cooldown:";
const STUCKNESS_COOLDOWN_TTL_SECONDS = 30 * 60; // ~5 cycles at average pace

function stucknessCooldownKey(outcomeName: string): string {
  return STUCKNESS_COOLDOWN_PREFIX + outcomeName.replace(/\s+/g, "-").slice(0, 120);
}
 ...
 */
async function pickStuckOutcome(
 ...
 */
async function buildStucknessAnchor(
  row: StucknessResult,
  eventBus: any,
): Promise<any> {
