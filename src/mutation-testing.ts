/**
 * Lightweight mutation testing — zero dependencies.
 *
 * After verification passes, applies simple mutations to changed source files
 * and re-runs tests. If tests still pass with a mutation, that code path has
 * no meaningful test coverage — a leading indicator of future reverts.
 *
 * Mutations applied (one at a time, restored after each):
 *   1. Negate boolean returns: `return true` to `return false` (and vice versa)
 *   2. Remove early returns: `return expr;` to a no-op comment
 *   3. Negate conditions: `if (x)` to `if (!(x))`
 *   4. Swap comparison operators: `===` to `!==`, `>` to `<`
 *
 * Design constraints:
 *   - 60-second time budget (configurable)
 *   - Only mutates files in the executor's diff (not the whole codebase)
 *   - Skips test files, config files, migrations, and type-only files
 *   - Never throws — returns a result object
 *   - Restores original file contents even on crash (try/finally)
 */
