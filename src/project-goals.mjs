 ...
 * Project Goals
 *
 * Loads and parses the project goals document from the vault.
...
const GOALS_FILE = join(HYDRA_PATH, "direction", "goals.md");
...
export async function loadProjectGoals() {
  ...
  for (const [key, lines] of Object.entries(sections)) {
    if (!knownSections.has(key)) {
      goals.customSections[key] = lines.join("\n").trim();
    }
  }
...
export function summarizeGoalsForPrompt(goals) {
  ...
  if (Object.keys(goals.weights).length > 0) {
    ...
    const sorted = Object.entries(goals.weights).sort((a, b) => b[1] - a[1]);
...
export async function loadAppMetrics() {
  ...
  const metricsFile = join(HYDRA_PATH, "metrics", "app-metrics.json");
  try {
