...
/**
 * Issue #2647 / #3006 — injectable deps for the plan-time knowledge route
 * (`GET /learning/knowledge`) this router owns. All optional; each defaults to
 * the real implementation (`deps?.field ?? realImpl`) so production mounts
 ...
 * deterministically without a live OpenViking / Redis connection. This narrow
 * three-field bag lets a knowledge-route test stub only the OV search seam plus
 * the two metric/ledger writers.
 */
...
/**
 * OpenViking proxy + knowledge metrics + plan-time knowledge fetch routes.
 *
 ...
 * the canonical `trackedOvSearch` reader. Issue #3006 folded the plan-time
 * knowledge fetch (`GET /learning/knowledge` + its #2647/#2717 side effects) in
 * here — this router already owns the Knowledge-Base HTTP surface
 * (`/openviking/search`, `/openviking-stats`, `/learning/coverage`, all wrapping
 * `trackedOvSearch` / `loadKnowledgeBaseForPrompt` from the same
 * `knowledge-base/ov-search.ts` module), so the fetch lands at its domain home
 * with flat fan-out (no new `src/api/knowledge.ts` file, no 4th factory).
 */
  ...
  router.get("/learning/knowledge", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          `[knowledge-api] knowledge availability record failed: ${recErr?.message ?? recErr}`,
        );
      }
      ...
      try {
        ...
        console.error(
          `[knowledge-api] knowledge ledger append failed: ${ledgerErr?.message ?? ledgerErr}`,
        );
      }

      res.json({ agent, content, itemCount });
    } catch (err: any) {
      console.error(`[knowledge-api] knowledge failed: ${err?.message || String(err)}`);
