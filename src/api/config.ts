...
export function createConfigRouter() {
  ...
  //
  // Not an isolateAggregator route: the success path responds with
  // `text/plain`, not a JSON body the seam's `res.json(produce())` can express.
  router.get("/config/:section/:name", async (req, res) => {
    ...
    try {
      ...
      logger.error(
        { routeLabel: "api/config/section/name", section: req.params.section, name: req.params.name, err },
        "[api/config] read failed",
      );
  ...
  router.put("/config/:section/:name", async (req, res) => {
    ...
    return isolateAggregator(res, "api/config/section/name/put", async () => {
  ...
  router.get<{ project: string }>("/env/:project", requireEnvAuth, async (req, res) => {
    ...
    return isolateAggregator(res, "api/env/project", async () => {
  ...
  router.put<{ project: string }>("/env/:project", requireEnvAuth, async (req, res) => {
    ...
    return isolateAggregator(res, "api/env/project/put", async () => {
  ...
  //
  // Not an isolateAggregator route: the success path returns a 404 for a
  // not-found key, which the seam (JSON-at-200 of produce's return) can't
  // express.
  router.delete<{ project: string; key: string }>("/env/:project/:key", requireEnvAuth, async (req, res) => {
    ...
    try {
      ...
      logger.error(
        { routeLabel: "api/env/project/key/delete", project: req.params.project, key, err },
