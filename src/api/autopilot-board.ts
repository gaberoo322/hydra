...
export function createAutopilotBoardRouter(deps: AutopilotBoardRouterDeps = {}) {
  ...
  router.get("/autopilot/board-state", async (req, res) => {
    ...
    // The GLM-withheld verdict list (issue #4254): `[]` on every degraded /
    // partition-inactive arm, populated ONLY alongside a healthy count below,
    // from the SAME `glmPartitionActive` value that count used.
    ...
    if (read.ok === true) {
      ...
      try {
        ...
        // Publish the per-issue withhold verdicts (issue #4254) from the same
        // rows and the same liveness value — `collect-state.sh` consumes this
        // list for its `orch_dev_ready_anchor` pick guard instead of mirroring
        // the label rule + heartbeat read in shell (#4253's drift class).
