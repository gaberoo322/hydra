...
// (issue #3505) — imported above. The eight pure /work + hitl-grill
// projections (`deriveWorkLane`, `toWorkQueueRow`, `compareWorkQueueRows`,
// `evaluatePromoteEligibility`, `computeRelabelTransitions`, `parseParkReason`,
// `toHitlGrillRow`, `compareHitlGrillRows`) live in the sibling
// `src/autopilot/work-projections.ts` leaf (issue #4408) — the same extraction
// applied to the /work feature. This router is a thin HTTP adapter that wires
// those projections onto the GitHub-Read seam: the projection math itself has
// no Express dependency, so it belongs in the `autopilot` domain, not here. A
// downstream multi-scope reader (`src/target-board-labels.ts`) imports the
// pure functions directly from those leaves, never from this route file.
//
