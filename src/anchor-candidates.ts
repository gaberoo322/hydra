...
// WorkQueueHygiene reconciler below is the other.
...
// injectable test seam — this only changes the default. (The MergedAnchorRefs
// loader's own `exec` seam moved to `src/backlog/merged-refs.ts` with the rest
// of that cluster in issue #1880.)
...
// `src/autopilot/pr-lifecycle-bridge.ts`). Used by the WorkQueueHygiene
// issue-state reader below.
...
// external fix). The merged-suppression above hides entries whose identity
// matches a recently-merged PR token, but an out-of-band resolution often has
// NO matching merged-PR token — the entry kept resurfacing at work-queue tier
// (0.70) and dev_target burned a full dispatch on no-op verify+LREM (highest-
// recurrence unfixed retro cue, recurrence 14). `reconcileWorkQueue` is the
// shared engine: it REMOVES entries that are (a) merged work per the #882
// token scan, or (b) reference orchestrator issues that are ALL closed. It is
// fail-open — any uncertainty (no issue refs, an open issue, an unreachable
// `gh`) keeps the entry. Drivers: the hourly `work-queue-hygiene` housekeeping
// chore and the operator-facing `POST /api/queue/reconcile` hook.
