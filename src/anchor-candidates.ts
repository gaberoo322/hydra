...
// Pure scoring arithmetic — the tier ladder, penalty/bonus weights, and the
// `scoreCandidate` function — now live in their own Redis-free module (issue
// #2040). This module owns the STATEFUL eligibility guards + enumeration below;
// it imports the scorer and re-exports its public symbols so the Candidate
// Feed's public interface is unchanged.
import {
  scoreCandidate,
  RESEARCH_THRESHOLD,
  ...
  type ScoreSignals,
  type ScoreResult,
} from "./anchor-scoring.ts";

// Re-export the scoring surface this module has always exposed (issue #2040 kept
// the public interface stable when the pure arithmetic moved to
// `anchor-scoring.ts`): consumers and tests that import these from
// `anchor-candidates.ts` keep working unchanged.
export {
  scoreCandidate,
  PRIORITY_TIER_BASE_SCORE,
};
export type { PriorityTier, ScoreSignals, ScoreResult };

// Eligibility threshold — STATEFUL guard, stays here (not scoring). A blocker is
// "just cleared" when its lane moved within this window.
