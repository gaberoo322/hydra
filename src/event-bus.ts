...
// ---------------------------------------------------------------------------
// Consumer-group lifecycle — the bus's own XGROUP/XINFO setup, teardown, and
// zombie reaping. These are the raw XGROUP / XINFO verbs the bus seam owns
// (ADR-0017 Category B: the Event Bus is the sanctioned raw-connection owner).
//
// They live here as module-level functions — NOT in a separate file — because
// consumer-group lifecycle does not vary independently of the bus that owns
// those groups (issue #2340 folded the former `event-bus-lifecycle.ts` back in).
// None of it runs in the hot consume loop, none of it depends on the bus's
// stream-topology constants, and each function takes a raw `Redis` client so it
// stays testable without instantiating a full bus. `EventBus` keeps one-line
// delegator methods that forward into these so every caller (`src/index.ts`,
// the slot-events bridge, the recs consumer, and the tests) stays zero-diff.
// ---------------------------------------------------------------------------
...
class EventBus {
   ...
   * module-level `initConsumerGroups` — group setup is an operational bootstrap
   * concern, not part of the bus's hot stream-transport path.
   */
   ...
   * module-level `ensureConsumerGroup`; kept on the class so callers
   ...
   * delegator to the module-level `reapStaleConsumers`, passing the bus's
   ...
   * (issue #1221). One-line delegator to the module-level `delConsumer`;
