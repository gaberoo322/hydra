...
// ---- Skill-catalog health gate (issue #1968) -----------------------------
//
// When OpenViking times out / 5xx's under indexing load during startup skill
// registration, all four OV skills can fail to register and the catalog stays
// empty — yet the service reports a clean startup and no health surface reflects
// it, so planners run without skill context (silent knowledge-plane failure).
//
// `assessSkillCatalog` is the pure verdict over the in-process skill-catalog
// state produced by `registerSkills` (src/knowledge-base/skill-registration.ts).
// Kept here in the pure seam — and decoupled from the producer's concrete type
// via a structural input shape — so it is unit-testable without standing up
// OpenViking, and so the health-diagnostics seam keeps its import-free, no-I/O
// discipline (#840). The I/O handler (api/health.ts) reads the live state and
// passes it in; this function never touches the registry module.
