/**
 * Autopilot recommendation-engine PROMPT-GRAMMAR leaf (issue #2867).
 *
 ...
 * plus the input/output types they operate over and the prompt-size budget.
 *
 * ## Why this is a standalone leaf
 *
 * This section was folded INTO `recommendation-engine.ts` by issue #2317 (it
 * had become a shallow pass-through named `recommendation-prompt.ts` under
 * #2240). Issue #2867 re-extracts it, because the fold left the prompt grammar
 * tangled with the firing-interval logic, the daily-cap ledger, and the
 * Anthropic Request Adapter — so a promptfoo A/B eval (`evals/`) that wants to
 * import `buildPrompt` directly had to drag the whole 817-line coordinator (and
 * its Redis + Anthropic transitive imports) in at module-load time.
 *
 * Everything in this module is PURE: no Redis, no network, no clock, no
 ...
 * promptfoo scorer pass a synthetic `EnginePromptInput` and call `buildPrompt`
 * with ZERO engine wiring — no `EngineDeps`, no cap ledger, no adapter.
 *
 * The engine coordinator (`recommendation-engine.ts`) imports from here and
 * re-exports the types + prompt functions it needs, so the `EngineDeps` /
 * `LlmResult` surface `recommendation-consumer.ts` consumes stays unchanged.
 */
...
// ---------------------------------------------------------------------------
// Prompt grammar — the pure prompt builder + response parser
// ---------------------------------------------------------------------------
