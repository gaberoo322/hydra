/**
 * Per-class yield scoreboard + shadow-mode dampener (issue #2943, follow-on to
 * the Outcome Attribution Spine epic #2628 and the per-dispatch outcome record
 * #2942).
 *
 * # What this closes
 *
 ...
 *
 * The scoring math ({@link computeClassScoreboard}, {@link shadowDampener}) is
 * PURE — deterministic functions of already-read inputs, no I/O, never throws.
 * The Redis reads live in {@link buildClassScoreboard}, the composer the
 * read-only API view calls. `decide.py` NEVER calls any of this: the scoreboard
 * is computed orchestrator-side and injected into `state.json` via
 * `collect-state.sh`; decide.py stays a pure function of `state.json` (the #2943
 * byte-identical-dispatch invariant).
 *
 * `src/outcome-attribution/estimator.ts` and `src/redis/attribution-ledger.ts`
 * are CONSUMED read-only here (files-out-of-scope for #2943) — this module
 * mutates neither.
 */
