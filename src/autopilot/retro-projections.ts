/**
 * Retro-bundle **pure projections** — the testable, side-effect-free derivation
 * surface that powers the per-run retrospective bundle (issue #918, epic #917).
 *
 * Split out of `src/autopilot/retro-bundle.ts` (issue #1952) so the
 * side-effectful assembler (`assembleRetroBundle` — Redis fan-out, never-throw
 * joins) and the pure `(turns) → RetroDispatch[]` projection logic live in
 * separate modules — mirroring the established `runs.ts` / `run-projections.ts`
 * split (issue #1183). This module owns the read-only, pure surface:
 *
