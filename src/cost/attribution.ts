...
/**
 * Authoritative model-name -> tier table. Keep in sync when adding a
 * new model id to the routing table — otherwise its spend lands in
 * `byTier` as "unknown" and the cost-attribution dashboard becomes
 * unreliable (issue #303: 38-48% of runs were classified "unknown"
 * because the planner was bumped to `gpt-5.5` but `modelToTier` only
 * matched 5.4).
 */
 ...
 * — every model id used by the autopilot subagents should be enumerated
 * in `MODEL_NAME_TO_TIER` above, so seeing "unknown" in production is a
