/**
 * adversarial.ts — Adversarial validation and finding-to-queue conversion
 *
 * Extracted from verification.ts (issue #161).
 *
 * Exports:
 *   - runAdversarialValidation()  — run adversarial validation on merged files
 *   - findingsToQueueItems()      — convert findings to work queue items
 *   - checkRevertCorrelation()    — check recent reverts for precision tracking
 *   - trackMergedCommit()         — record a merge for revert-correlation
 *   - AdversarialFinding type     — finding shape
 *   - AdversarialReport type      — report shape
 */
