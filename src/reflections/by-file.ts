 ...
 *
 * Redis key shape: `hydra:reflections:by-file:{filepath}` (a SET of anchor keys),
 ...
 *
 * Returns the list of files indexed (empty on no-match or any error) so the
 * caller can log the file names — restoring the pre-split log detail (issue
 * #1938: the count-only return dropped the `"by N file(s): file1, file2…"`
 * detail the former `reflections.ts` emitted). Use `.length` for a count.
 */
