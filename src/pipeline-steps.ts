...
import { recordOutcome, clearOutcomes } from "./learning.ts";
...
export async function handlePlanResult(
  ...
  if (!task) {
    ...
    await recordOutcome("planner", cycleId, null, {
      finalState: "no-task",
      reason: "Planner could not produce a valid task from this anchor",
      anchor,
...
export async function runExecuteStep(
  ...
  if (!hasDiff) {
    ...
    await recordOutcome("planner", cycleId, task, {
      ...
      taskTitle: task.title,
      context: {
        failReason: "Executor produced no code changes",
