/**
 * repo-import-graph.ts — Regex-based TypeScript parser for exported symbols and
 * import edges, plus the import-graph / PageRank machinery built on top of it.
 *
 * Builds an adjacency graph (file A imports file B) from .ts/.tsx source files.
 * Zero runtime dependencies — regex only.
 *
 * A per-file-tree-hash cache (`clearRepoMapCache()`) lets callers reuse a built
 * graph across repeated calls within the same grounding cycle (unchanged
 * project) for free.
 *
 ...
 *
 * This is the single home for the test-file predicate; the matcher and
 * tree-formatter concerns import it from here (issue #805 — no duplication).
 */
