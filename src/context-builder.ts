 ...
 *
 * Depends on: learning, metrics, grounding, backlog (dynamic),
 *             ioredis (for continuity), redis-keys
 */
...
async function loadContinuityContext(
  ...
  // Reflections (per-anchor + global) are now loaded via getContext() in
  // buildPlannerContext and included in plannerMemory — no separate load needed.
