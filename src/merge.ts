...
export async function mergeToMain(projectDir, cycleId) {
  try {
    ...
    if (featureBranch && featureBranch !== "main") {
      ...
      // Delete the merged feature branch locally and remotely (non-fatal).
      try {
        ...
        console.error(
          `[Merge] Failed to delete local branch ${featureBranch}: ${err.message}`,
        );
      }
      try {
        await execFileAsync("git", ["push", "origin", "--delete", featureBranch], {
          cwd: projectDir,
          timeout: 15000,
        });
      } catch (err) {
        /* intentional: remote branch may not exist or already be deleted */
      }
