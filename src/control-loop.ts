...
export async function runControlLoop(eventBus: any, opts: Record<string, any> = {}) {
  ...
  const { verification, reconciliation, mutationReport, jitReport, fixerUsed, fixerResolved, mutationDecision, mutationFilesInspected, groundingMode, incrementalTestsSelected, incrementalReason } = vResult;
  ...
  const { report } = await runPostMerge(
    ...
    { mutationDecision, mutationFilesInspected, groundingMode, incrementalTestsSelected, incrementalReason },
  );
