...
The in-process loop in `src/scheduler/loop.ts` that ticks every 15 minutes while `hydra-orchestrator.service` is running. Continuously alive — distinct from the **Autopilot Run**, which is a discrete timer-fired session. Per the architecture decision recorded in the next ADR ("scheduler is bookkeeping; autopilot is decisions"), the Scheduler is **strictly observability-and-counters only**: it maintains `lastTickAt` (watchdog liveness), rolling merge-rate windows, daily-spend totals, and cycle-record aggregation. It does NOT make policy decisions, dispatch work, or mutate kanban/work-queue state based on rules. The historical research-floor and stale-claim-reaper logic moves into `decide.py` (the **Autopilot Run** owns all decisions about *what to do*; the Scheduler only records *what happened*). The Scheduler's name is preserved for now but its surface is converging on a heartbeat-plus-counters service — future rename to **Observability Heartbeat** is plausible.
