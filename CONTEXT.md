...
**Cost**:
The `src/cost/` Module that owns Claude Code subagent spend accounting on the orchestrator side — recording per-skill / per-cycle token usage, exposing the daily-spend surrogate the autopilot consults each tick (`Daily spend: $X/$50`), and aggregating per-tier attribution for the dashboard. Stores everything through the **Redis Adapters** under `hydra:cost:*`. Accounting only — daily-spend **enforcement** lives in the autopilot's decision brain (`scripts/autopilot/decide.py` reads `state.scout_spend_usd_today` vs `state.limits.daily_spend_cap_usd`), not in this Module. The pre-codex per-cycle circuit breaker (`checkCostCap`) and the Codex JSONL reconciliation pipeline (`cost-reconciliation.ts`, `/api/cost/reconciliation`) were retired with ADR-0006.
_Avoid_: cost-cap (the retired per-cycle name), cost-surrogate / cost-attribution (file names, not the Module), "spend tracking" (too narrow — also owns attribution + cap config)
