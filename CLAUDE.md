...
- **An exported shell var does not survive into a `gh ... --body-file /dev/stdin <<EOF` heredoc**: a `gh pr create --body-file /dev/stdin <<EOF` (or any command reading a heredoc) runs in a subshell that does NOT see your `CYCLE_ID` / branch-name vars — the body lands with a literal empty/`$CYCLE_ID` instead of the value (bit a retro/PR-create cycle). Inline the actual value into the heredoc text (hardcode the branch name / interpolate before the heredoc), or build the body with a here-string into a temp file first; do not rely on exported vars expanding inside the redirected heredoc.
