 ...
 *
 * Background: `hydra-discover`, `hydra-architecture-scan`, and (operator-only)
 * `hydra-research` each turn idle capacity into GitHub issues. The two that
 * matter for autonomous collisions — `hydra-discover` and
 * `hydra-architecture-scan` — BOTH fire on the SAME unified `orch_backfill_idle`
 * signal (discover_orch revived #959; both in `BACKFILL_SIGNAL_CLASSES`,
 * decide.py:329). The one-per-turn stagger guard stops them dispatching the same
 * TURN, but their independent per-class 1h cooldowns plus the
 * `BACKFILL_STARVATION_FLOOR` mean both can fire within the same idle HOUR.
 *
 * Before this helper each skill carried its OWN prose copy of the dedup rule
 * ("word overlap >50% → SKIP") with no shared, testable implementation — and,
 * worse, each only deduped against ITS OWN label set (discover reads
 * `needs-triage`; architecture-scan reads open `architecture-scan` issues;
 * cleanup reads `cleanup-scan`). Neither saw the OTHER's freshly-filed issues,
 * so the same idle window could surface a discover `needs-triage` issue AND an
 * architecture-scan issue describing the same underlying gap — the live
 * "backfill dedup baseline" double-file path (issue #2554, design-concept
 * `issue-2554`).
 *
 * This module is the single source of truth for the duplicate judgement: a
 * pure, deterministic word-overlap test the playbooks call (via the CLI below)
 * so EVERY board-filling skill computes "duplicate?" the exact same way against
 * the exact same shared baseline (open issues across ALL backfill label sets +
 * issues closed within the dedup window). When the skills agree byte-for-byte
 * on what counts as a duplicate AND read each other's in-flight issues, the
 * same finding cannot slip past one and get filed by the other.
 *
 ...
 * (the shared baseline = open issues across every backfill label set + issues
 * closed within the dedup window). Returns the best (highest-overlap) match so
 * the caller can log it verbatim. Pure — never throws on well-formed string
 * input.
 *
