...
# them here, merges them under the `slot_events` JSON key, and
# `decide.py` consumes them to free slots without polling. The cursor is
# `state.slot_events_last_id` — the autopilot is expected to update it
# after each successful read so the next turn doesn't re-process the
# same events.
#
# Issue #4510: this used to shell out to `docker exec hydra-redis-1
# redis-cli XREAD` and hand-parse the plain-text reply through a
# ~30-line regex heuristic — a second, drift-prone re-derivation of the
# wire format the typed EventBus (`src/event-bus.ts`'s `readRaw()`)
# already owns correctly. It now reads the same projection through the
# HTTP seam (`GET /autopilot/slot-events`), exactly like
# `collect_orch_board` / `collect_retro` already do.
#
# Best-effort: a Redis outage or empty stream prints the endpoint's own
# safe-default JSON under `slot_events_json=`. The collect step never fails.
collect_slot_events() {
...
hydra raw GET "/autopilot/slot-events?last_id=${SLOT_EVENTS_LAST_ID}&count=${SLOT_EVENTS_COUNT}" 2>/dev/null || echo '{"events": [], "last_id": null}'
}
