...
# Issue #4398: the `gh` subprocess seam, REPO/TARGET_REPO repo constants, and
# the PR-reference/closing predicates all moved to the prerequisite leaf
# reap_ghrefs.py (absorbed from #4366's originally-separate reap_ghrefs
# slice — see that module's docstring for why reap_stall.py needs it as a
# leaf reap.py never imports, INV-3). `_gh_run` is bound here even though
# reap.py's own run_completion body no longer calls it directly (every
# remaining call site lives in reap_ghrefs.py/reap_stall.py now) — pinned
# test/autopilot-dev-resume-stall.test.mts probes it as `reap._gh_run(...)`
# and monkeypatches `reap.subprocess.run` expecting the SAME shared
# `subprocess` module object to reach reap_ghrefs's `_gh_run` (INV-9); a
# name-binding import here is what keeps that resolving unedited.
...
# Issue #4398: the three dev_orch/dev_target stall-recovery handlers moved to
# reap_stall.py — see that module's docstring for the invariant contract
# (verbatim move, no signature/body changes, same three run_completion call
# sites below).
from reap_stall import (
    _handle_dev_orch_needs_qa_promotion,
...
def run_completion(cls: str, task_id: str, total_tokens: int, skill: str | None, last_segment_tokens: int | None = None) -> int:
    ...
    # structurally dead-arm the whole class against ADR-0031 Decision 4's
    # WIP cap of 3 — a deadlock, not a leak. Release a confirmed stall
