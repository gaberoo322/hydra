...
Greppable IDs (also asserted by `test/autopilot-invariants.test.mts`):
  ...
  INV-001  RETIRED (ADR-0020 Slice 2 / #743) — was "never auto-merge a T4
           (Verifier Core) PR". A plan-level guard cannot see the Deep-QA
           PASS marker, so it could not do its job once T4 *may* auto-merge.
           The T4 depth backstop relocated to the base-ref `deep-qa-gate`
           required CI check + retained INV-007.
...
# INV-001 ("never auto-merge a T4 Verifier-Core PR") is RETIRED — ADR-0020
# Slice 2 (#743). Once T4 *may* auto-merge and decide.py cannot see the
# Deep-QA PASS marker, a plan-level guard could no longer do its job (a defense
# that cannot see the thing it defends is theater). The T4 depth backstop
# relocated to the base-ref `deep-qa-gate` required CI check (the SHA-bound
# Deep-QA PASS marker) plus the retained INV-007. Do NOT reintroduce it.
...
ALL_CHECKS = (
    # INV-001 retired (ADR-0020 Slice 2 / #743) — see note above check_inv002.
