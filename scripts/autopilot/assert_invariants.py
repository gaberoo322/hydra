...
def check_inv001(actions: list[dict]) -> None:
    ...
    Renumbered from the old Tier-0 (ADR-0015 / issue #737): the deepest
    tier — Verifier Core — moved from integer 0 to 4. Identical guard
    semantics; only the integer naming the deepest tier moved.
    """
    for a in actions:
        ...
        try:
            ...
            tier = -1
        if tier == 4:
            raise InvariantError("INV-001", f"auto-merge on T4 (Verifier Core) PR #{a.get('pr_number')}")
