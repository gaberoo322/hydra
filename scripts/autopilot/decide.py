...
# `decide.py` is normally invoked as a script (`__name__ == "__main__"`), but
# the per-class selector leaves under `dispatch_selectors/` (issue #4511) do
# `from decide import <helper>` so they resolve against the SAME running
# module rather than each re-importing (and re-executing) this file from
# scratch. Registering this module under its importable name "decide" makes
# that resolve to the module already executing, picking up whatever helpers
# are already defined by the time a selector leaf is imported below — no
# re-exec, no duplicate `Plan`/`TaxonomyError` classes. A no-op when this
# file is already being imported AS "decide" (e.g. heartbeat.py's existing
# `from decide import SIGNAL_COOLDOWNS` sibling-import, or
# test/taxonomy-classes.test.mts) since `sys.modules["decide"]` is then
# already this same module and `setdefault` changes nothing.
sys.modules.setdefault("decide", sys.modules[__name__])

# `import dispatch_selectors.<x>` below needs this file's own directory on
# sys.path — true automatically when decide.py is run as a script or
# imported via `sys.path.insert(0, dir); import decide` (heartbeat.py,
# test/taxonomy-classes.test.mts), but NOT when a caller loads it by literal
# file path (`importlib.util.spec_from_file_location`, as
# test/decide-signal-classes.test.mts's `probeIsolation` helper does) —
# sys.path is left untouched in that case. Insert explicitly so
# dispatch_selectors resolves regardless of how decide.py itself was loaded.
...
# Handler bodies live under dispatch_selectors/ (issue #4511) — decide.py
# imports each and wires it into the registry below; this stays the ONE
# place dispatch is actually decided from.
from dispatch_selectors.qa import _select_slot_qa_orch, _select_slot_qa_target
from dispatch_selectors.dev import _select_slot_dev_orch, _select_slot_dev_target
from dispatch_selectors.research import (
    _select_slot_research_orch,
    _select_slot_research_target,
)
from dispatch_selectors.design_concept import _select_slot_design_concept_orch
...
# imports each and wires it into the registry below; this stays the ONE
# place dispatch is actually decided from.
from dispatch_selectors.health import _select_signal_health
from dispatch_selectors.sweep import (
    _select_signal_sweep_orch,
    _select_signal_sweep_target,
)
from dispatch_selectors.discover import (
    _select_signal_discover_orch,
    _select_signal_discover_target,
)
from dispatch_selectors.scout import _select_signal_scout_orch
from dispatch_selectors.architecture import _select_signal_architecture_orch
from dispatch_selectors.retro import _select_signal_retro_orch
from dispatch_selectors.cleanup import (
    _select_signal_cleanup_orch,
    _select_signal_cleanup_target,
)
from dispatch_selectors.wire_or_retire import _select_signal_wire_or_retire_target
from dispatch_selectors.design_qa import _select_signal_design_qa_target
from dispatch_selectors.skill_prune import _select_signal_skill_prune
from dispatch_selectors.wayfinder import _select_signal_wayfinder_orch
from dispatch_selectors.tickets import _select_signal_tickets_orch
