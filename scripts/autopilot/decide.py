...
# ---------------------------------------------------------------------------
# Per-class signal handlers (issue #4265). Each handler below owns exactly
# one dispatch class's gating rules; _SIGNAL_SELECTORS is the registry
# _select_for_signal looks the class up in AFTER the shared signal_is_cooled
# guard. Extraction is behavior-preserving: every branch body moved verbatim
# from the pre-#4265 shared function.
# ---------------------------------------------------------------------------
...
def _select_signal_health(
    ...
    """`health` signal-class dispatch selector (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_signal_sweep_orch(
    ...
    """`sweep_orch` signal-class dispatch selector (#3939, #3729, #3709, #2426) (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_signal_sweep_target(
    ...
    """`sweep_target` signal-class dispatch selector (#3709, #3729, #631, #626) (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_signal_discover_orch(
    ...
    """`discover_orch` signal-class dispatch selector (#959, #958, #4114, #4391) (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_signal_discover_target(
    ...
    """`discover_target` signal-class dispatch selector (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_signal_scout_orch(
    ...
    """`scout_orch` signal-class dispatch selector (#485, #483, #486) (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_signal_architecture_orch(
    ...
    """`architecture_orch` signal-class dispatch selector (#790, #787, #959, #958) (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_signal_retro_orch(
    ...
    """`retro_orch` signal-class dispatch selector (#920, #917, #919, #1078) (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_signal_cleanup_orch(
    ...
    """`cleanup_orch` signal-class dispatch selector (#960, #958) (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_signal_cleanup_target(
    ...
    """`cleanup_target` signal-class dispatch selector (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_signal_wire_or_retire_target(
    ...
    """`wire_or_retire_target` signal-class dispatch selector (#2722, #2720, #2721, #4411) (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_signal_design_qa_target(
    ...
    """`design_qa_target` signal-class dispatch selector (#2739, #2732, #2720, #2575) (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_signal_skill_prune(
    ...
    """`skill_prune` signal-class dispatch selector (#2949, #2944, #2575, #1078) (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_signal_wayfinder_orch(
    ...
    """`wayfinder_orch` signal-class dispatch selector (#3351, #3350, #2575, #1093) (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_signal_tickets_orch(
    ...
    """`tickets_orch` signal-class dispatch selector (#3423, #3419, #3992, #4014) (moved from
    the shared _select_for_signal god-function, issue #4265).
    """
...
def _select_for_signal(sig: str, state: dict, events: list[dict], now: int) -> dict | None:
    """Return a dispatch action for signal class `sig`, or None to idle.

    The shared `signal_is_cooled` cooldown guard stays here, BEFORE the
    registry lookup (issue #3729/#3939 INV-1: class cooldown is a necessary
    independent condition no handler re-checks or bypasses). Looks `sig` up
    in `_SIGNAL_SELECTORS` (issue #4265) and delegates to the per-class
    handler; an unregistered class returns None exactly like the pre-#4265
    fall-through.
    """
    if not signal_is_cooled(state, sig, now):
        return None
    fn = _SIGNAL_SELECTORS.get(sig)
    return fn(sig, state, events, now) if fn else None
