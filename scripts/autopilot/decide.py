...
state.json {
  ...
  # grows unbounded. Absent/empty = cold start = every item immediately eligible.
...
# That verdict thrash is the real defect; parking was falsified (both items left
# the lane within ~28h, well inside their grace windows).
#
...
def _target_triage_items_value(state: dict, events: list[dict]):
    ...
    `_parse_target_triage_items`. None is the fail-open sentinel consumed by
    the sweep_target guard: an absent item list means collect-state.sh did not
    emit the per-turn fact (legacy state, or a degraded board read), so the
    per-item guard goes dormant and sweep_target fires on the pre-#3729
    `needs_triage_target` rule alone — absent-safe like every existing state
    field decide.py reads via `.get(..., default)`.
    """
    for ev in events:
...
def _parse_target_triage_items(raw) -> set | None:
    ...
    None (absent / empty / wholly unparseable) is the fail-open sentinel. A
    present-but-empty list ("needs_triage_target true but items=''") is treated
    as absent too: the count and the item list derive from the same Target
    board read, so an empty list against a true signal is a stale/edge state,
    not "zero items" — fail-open toward firing rather than deadlock the lane.
    """
    if raw is None:
        return None
...
def _select_for_signal(sig: str, state: dict, events: list[dict], now: int) -> dict | None:
    ...
    if sig == "sweep_target":
        ...
        # When collect-state.sh emits the per-turn item-number list, suppress
        # the dispatch if EVERY current item was examined within the backoff
        # window. Absent list (None) -> fail-open: the guard goes dormant and
        # sweep_target fires on the pre-#3729 needs_triage_target rule alone.
        items = _parse_target_triage_items(_target_triage_items_value(state, events))
        if items is not None and not _target_triage_any_eligible(state, items, now):
            return None
        ...
        # No-op when the item list is absent (nothing to stamp; the guard was
        # dormant). sweep_target is not a BACKFILL_SIGNAL_CLASSES member, so the
