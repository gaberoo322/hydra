...
def _rule_termination(state: dict, events: list[dict], now: int) -> _RuleOutput:
    ...
    """
    out = _RuleOutput()
    # `events` is threaded for the #4130 idle guard (a degraded orch board read
    # suspends the idle-drain conclusion inside _check_termination).
    term = _check_termination(state, events, now)
    if term is None:
        return out
...
def _rule_idle_fallback(
    state: dict, events: list[dict], *, dispatched_any: bool, plan_has_actions: bool
) -> _RuleOutput:
    ...
    Issue #4130: that idle conclusion is SUSPENDED while the orch board read
    is degraded. "Wait-only turn, no slots" is exactly what a gh/GraphQL
    outage manufactures (every board signal reads empty/absent), so the
    #1352 terminate would end the run with a clean `idle` cause for what is
    really an outage. Instead the turn takes the heartbeat wait with a
    DISTINCT reason — `orch board read degraded — idle conclusion suspended`
    — which lands in the plan reasons[] and the heartbeat turn record, so a
    held drain is observable and distinguishable from a genuinely drained
    board in the audit trail (issue #4130 AC-4). The normal drain resumes
    once reads recover.
    ...
    degraded = _orch_board_read_degraded(state, events)
    if not dispatched_any and occupied == 0 and not plan_has_actions and not degraded:
        out.emit(
            make_terminate(
                "idle",
    ...
    elif not dispatched_any and occupied == 0 and not plan_has_actions and degraded:
        # #4130 degraded hold — same wait the heartbeat branch below emits,
        # but with a reason that names the outage. `orch_board_degraded_hold`
        # in debug mirrors `idle_fallback`'s shape for dashboard/audit reads.
        out.emit(
            make_wait(
                WALL_CLOCK_HEARTBEAT_SEC,
                "orch board read degraded — idle conclusion suspended (issue #4130)",
            ),
            reason="degraded-board-hold",
        )
        out.debug["idle_fallback"] = "degraded-hold"
        out.debug["orch_board_degraded_hold"] = True
...
def decide(
    ...
    term_out = _rule_termination(state, events, now)
    fold(term_out)
    ...
    #    (merges / reaps / queue-decisions) without dispatching. `events` is
    #    threaded for the #4130 degraded-board hold (a failed orch board read
    #    suspends the idle conclusion and takes the heartbeat wait instead).
    fold(
        _rule_idle_fallback(
            state,
            events,
...
def _select_for_signal(sig: str, state: dict, events: list[dict], now: int) -> dict | None:
    ...
    if sig == "discover_orch":
        # Issue #4130: a degraded orch board read must not arm EITHER trigger
        # below — not the idle backfill (an unreadable board is not an empty
        # board) and not the staleness floor (the discovery pass reads the
        # board to file issues; filing against a board it cannot see is the
        # duplicate-flood hazard). Hold one turn; the outage is per-turn.
    ...
    if sig == "architecture_orch":
        ...
        #
        # Issue #4130: a degraded orch board read fails closed here too. This
        # was the live hazard on 2026-08-17 — the ARCH read's old zero-JSON
        # fallback computed `orch_backfill_idle=true` from a FAILED read and
        # this arm dispatched architecture backfill at a board it could not
        # see. collect-state.sh now fail-closes the signal itself; this guard
        # is the second lock (belt and braces for a stale/replayed signal).
        if _orch_board_read_degraded(state, events):
            return None
    ...
    if sig == "cleanup_orch":
        ...
        #
        # Issue #4130: fail closed on a degraded orch board read, exactly like
        # architecture_orch above — never arm the dead-code sweep off a board
        # this turn could not see (collect-state.sh fail-closes the signal
        # itself; this is the second lock).
        if _orch_board_read_degraded(state, events):
            return None
    ...
    if sig == "skill_prune":
        ...
        #
        # Issue #4130: rides orch_backfill_idle, so it fails closed on a
        # degraded orch board read like every other backfill class above.
        if _orch_board_read_degraded(state, events):
            return None
...
def _orch_board_read_degraded(state: dict, events: list[dict]) -> bool:
    """True when collect-state.sh flagged ANY orchestrator board read as failed.

    Issue #4130. `orch_board_signals_degraded` is emitted unconditionally by
    collect-state.sh (explicit `false` when every read succeeded, `true` when
    any of the ten orch-lane board reads exited non-zero). decide.py stays
    PURE here — it reads the pre-resolved flag and never re-derives board
    health (signal-seam discipline, mirrors how `target_board_signals_degraded`
    is consumed).

    Callers: the idle-termination paths (`_check_termination`,
    `_rule_idle_fallback`) suspend the "board is empty -> drain" conclusion
    on a degraded read (an outage must not masquerade as a drained board),
    and the orch_backfill_idle backfill selectors (`discover_orch`,
    `architecture_orch`, `cleanup_orch`, `skill_prune`) never fire off a
    board they could not see.
    """
    return _signal_present(state, events, "orch_board_signals_degraded")
...
def _check_termination(state: dict, events: list[dict], now: int) -> dict | None:
    ...
    `events` is needed for the #4130 idle guard below (the degraded flag lives
    on the signal/event seam, not on state).
    """
    limits = state.get("limits") or {}
    ...
    # Issue #4130: the idle conclusion requires a board view this turn could
    # actually SEE. During a degraded orch board read (gh/GraphQL outage) every
    # board signal reads as empty/absent, wait-only turns pile up, and
    # idle_turns climbs — terminating on that counter would end the run with a
    # clean-looking `idle` cause for what is really an outage (the 2026-08-17
    # failure). Hold instead (return None here and let the idle-fallback rule
    # below emit its degraded-hold WAIT, whose reason is what lands in the
    # heartbeat turn record); the drain conclusion resumes once reads recover.
    # Scoped to the idle cause ONLY — budget / wall_clock / quota /
    # failure_backstop remain live during an outage (those are measured, not
